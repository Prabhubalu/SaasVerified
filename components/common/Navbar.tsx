        ...
        <div className="max-w-full mx-auto px-1 sm:px-2 md:px-3 lg:px-4 xl:px-6 py-2">
          <div className="flex justify-between items-center gap-1 sm:gap-2 lg:gap-4 min-w-0">
            ...
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 xl:gap-6 flex-shrink-0 min-w-0">
              ...
              {!showLogout && (
                <>
                  <div className="hidden lg:flex xl:hidden items-center gap-4 flex-shrink-0">
                    {primaryLinks.map((link) => (
                      <Link
                        key={link.href}
                        href={link.href}
                        className="text-gray-800 hover:text-gray-900 whitespace-nowrap text-sm lg:text-base"
                      >
                        {link.label}
                      </Link>
                    ))}
                    {moreLinks.length > 0 && (
                      <div className="relative" ref={moreMenuRef}>
                        <button
                          ...
                          className="text-gray-800 hover:text-gray-900 whitespace-nowrap text-sm lg:text-base flex items-center gap-1"
                        >
                          More
                          <ChevronDownIcon className="w-4 h-4 text-gray-400" aria-hidden="true" />
                        </button>
                        {isMoreOpen && (
                          <div className="absolute right-0 mt-2 w-40 rounded-lg border border-gray-200 bg-white shadow-lg overflow-hidden z-10">
                            {moreLinks.map((link) => (
                              <Link
                                key={link.href}
                                href={link.href}
                  ...
                  <div className="hidden xl:flex items-center gap-4 xl:gap-6 flex-shrink-0">
                    {navLinks.map((link) => (
                      <Link
                        key={link.href}
                        href={link.href}
              ...
              {showLogout ? (
                ...
                <>
                  <button
                    ...
                    className="text-[#12b76a] px-2 md:px-3 lg:px-3 xl:px-6 py-2 md:py-2.5 lg:py-3 text-xs md:text-sm lg:text-sm xl:text-base font-normal hover:text-[#12b76a] hover:bg-[#12b76a]/10 hover:underline transition-all rounded-full whitespace-nowrap"
                  >
                    Become a Verified Vendor
                  </button>
                  <Link href="/marketplace" className="bg-[#12b76a] text-white px-3 md:px-4 lg:px-4 xl:px-6 py-2 md:py-2.5 lg:py-3 rounded-full text-xs md:text-sm lg:text-sm xl:text-base font-normal hover:bg-green-700 transition-colors flex items-center gap-1 lg:gap-1.5 shadow-lg whitespace-nowrap">
            ...
            {showLogout ? (
              <button
                onClick={handleLogout}
                className="lg:hidden bg-red-600 text-white px-4 py-2 rounded-full text-sm font-normal hover:bg-red-700 transition-colors flex items-center gap-1.5 shadow-lg whitespace-nowrap"
              >
